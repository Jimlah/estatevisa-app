...
const Board = ({ title, icon, value, path }) => {
    ...
    return (
        <div className='w-full flex flex-col bg-white shadow-md overflow-hidden rounded-md my-5'>
            <div className='h-36 flex w-full flex-col p-6 space-y-4 items-start'>
                <span className='p-2  text-white' style={{ backgroundColor: color[rand] }}>
                    {icon}
                </span>
            ...
            <Link to={path} className={`px-6 py-4 text-white hover:bg-purple-600`} style={{ backgroundColor: color[rand] }} >
                View all
            </Link>
        </div>
    );
